...
describe("code graph query observability", () => {
  ...
  test("schedules lightweight embedding probe retries for completed RAG indexes", () => {
    ...
    const retryBody = serviceSource.slice(retryStart, retryEnd)
    expect(serviceSource).toContain("ragEmbeddingProbeRetryDecision")
    expect(serviceSource).toContain("private ragProbeRetryTimer")
    expect(serviceSource).toContain("private ragProbeRetryInFlight")
    expect(serviceSource).toContain("private scheduleRagEmbeddingProbeRetry")
    expect(serviceSource).toContain("private async runRagEmbeddingProbeRetry")
    expect(serviceSource).toContain("const completedIndex = pendingChunkCount === 0")
    expect(serviceSource).toContain("!input.index.staleReason")
    expect(serviceSource).toContain("!input.index.indexPausedReason")
    expect(serviceSource).toContain("resumeReason: \"probe\"")
    expect(serviceSource).toContain("[rag-index] embedding probe retry scheduled")
    expect(serviceSource).toContain("[rag-index] embedding probe retry starting")
    expect(serviceSource).toContain("[rag-index] embedding probe recovered")
    expect(serviceSource).toContain("trigger: \"index-load\"")
    expect(retryBody).toContain("await this.probeRagEmbeddingProvider(index.dimension)")
    expect(retryBody).not.toContain("refreshRagIndex")
    expect(retryBody).not.toContain("queuePendingRagRefresh")
    expect(typesSource).toContain('"probe"')
  })
  ...
  test("keeps code graph pause global while RAG-only pause avoids the job queue", () => {
    ...
    const runEnd = serviceSource.indexOf("private isCodeGraphReadyForRag", runStart)
    const runBody = serviceSource.slice(runStart, runEnd)
    expect(runBody).toContain("this.isManualRagIndexPaused()")
    expect(runBody).toContain("if (this.paused)")
    expect(runBody).toContain("pending RAG work preserved")
    expect(runBody.indexOf("this.isManualRagIndexPaused()")).toBeLessThan(runBody.indexOf("const pending = this.pendingRagRefresh"))
    expect(runBody.indexOf("if (this.paused)")).toBeLessThan(runBody.indexOf("const pending = this.pendingRagRefresh"))

    const refreshStart = serviceSource.indexOf("private async refreshRagIndex")
    const refreshEnd = serviceSource.indexOf("private abortRagIndex", refreshStart)
    const refreshBody = serviceSource.slice(refreshStart, refreshEnd)
    expect(refreshBody).toContain("this.isManualRagIndexPaused()")
    expect(refreshBody).toContain("pending RAG work preserved while RAG indexing is manually paused")
    expect(refreshBody.indexOf("this.isManualRagIndexPaused()")).toBeLessThan(refreshBody.indexOf("const pending = this.pendingRagRefresh"))
    expect(refreshBody.indexOf("this.isManualRagIndexPaused()")).toBeLessThan(refreshBody.indexOf("if (this.paused)"))

    const scheduleStart = serviceSource.indexOf("private schedulePendingRagWorkAfterCodeGraphReady")
    const scheduleEnd = serviceSource.indexOf("private runPendingRagRefreshWhenReady", scheduleStart)
    const scheduleBody = serviceSource.slice(scheduleStart, scheduleEnd)
    expect(scheduleBody).toContain("this.isManualRagIndexPaused()")
    expect(scheduleBody).toContain("pending RAG work preserved while RAG indexing is manually paused")

    expect(serviceSource).toContain("private async pauseActiveRagIndex")
    expect(serviceSource).toContain("private isManualRagIndexPaused")
  ...
  test("guards automatic RAG resume for stale cross-version partial indexes", () => {
    ...
    expect(serviceSource).toContain("validateRagCrossVersionPartialResume")
    expect(serviceSource).toContain("cross-version partial validated for safe resume")
    expect(serviceSource).toContain("cross-version partial resume rejected")
    expect(serviceSource).toContain("index.state === \"stale\" || index.staleReason")
    expect(serviceSource).toContain("resumeCompatibility === \"cross-version-validated\"")
    ...
    expect(serviceSource).toContain("queuePendingRagRefresh(undefined, { continuePreviousElapsed: true })")
  })
})
