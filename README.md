...
- Word / Diagram 辅助：支持 `documents` skill 驱动的通用 Word `.docx` 创建/编辑工作流、`read_docx` 语义读取、`inspect_word_document`/`apply_word_document_edits` 受控编辑、`create_word_document` 生成 preset-aware `.docx` 文档、`chipmate_render_mermaid_diagram` 通过远端 render server 生成 Mermaid `.mmd`/`.png` artifact，以及 draw.io / diagrams.net fenced block 的本地渲染和导出。
...
- `chipmate_render_mermaid_diagram`：当模型需要把 Mermaid 图嵌入 Word 或保留图表 artifact 时，先写出 `.chipmate/docs/diagrams/*.mmd`，再调用配置的远端 ChipMate render server `/render/mermaid` 生成 PNG；远端不可用或失败时会记录原因并 fail closed，不会用 Mermaid 源码冒充 Word 图。Word 插图场景可传 `scale: 3` 获取高 DPI PNG；工具返回的 `width`/`height` 是 Word 显示尺寸，`pixelWidth`/`pixelHeight` 是实际 PNG 像素尺寸。成功时返回 `*.png` 路径、尺寸、FigureSpec-compatible image 字段、render provider 诊断和聊天内 Mermaid 预览 artifact；不调用在线 Mermaid 服务。
