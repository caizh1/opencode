...
export async function getParserForFile(filepath: string) {
  try {
    ...
    await Parser.init({
      locateFile: (filename: string) => findExistingPath(runtimeWasmCandidates(filename)) ?? filename,
    })
...
export async function getQueryForFile(filepathOrUri: string, queryPath: string): Promise<Query | undefined> {
  ...
  const candidatePaths: string[] = [
    ...
    // ChipMate development/production: tree-sitter queries are packaged under vendor.
    path.join(repoRoot, "..", "vendor", "qwen-autocomplete", "tree-sitter", queryPath),
...
async function loadLanguageForFileExt(fileExtension: string): Promise<Language> {
  ...
  const candidatePaths: string[] = [
    ...
    // ChipMate development/production: language WASM files are packaged under vendor.
    path.join(repoRoot, "..", "vendor", "tree-sitter", "wasm", filename),
...
function runtimeWasmCandidates(filename: string): string[] {
  const repoRoot = path.resolve(__dirname, "..", "..", "..", "..", "..")
  return [
    path.join(repoRoot, "..", "node_modules", "web-tree-sitter", filename),
    path.join(repoRoot, "..", "vendor", "tree-sitter", filename),
    path.join(__dirname, filename),
  ]
}
