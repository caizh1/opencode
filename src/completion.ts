...
import { CompletionModelClient, completionModel, directCompletionRequestDiagnostic, type CompletionModelMessage } from "./completion-model-client"
...
export class RemoteCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  async provideInlineCompletionItems(
    ...
    const start = this.requests.request({
      ...
      runRemote: (signal) => this.directCompletionOutcome({
        document,
        position,
        settings,
        details,
  ...
  private async completionOutcomeWithRetry(input: {
    ...
    telemetry: CompletionTelemetryDraft
    selectedCompletionInfo?: SelectedCompletionInfo
    sendPrompt: (prompt: string) => Promise<CompletionModelMessage | undefined>
    selectedContextText?: string
  }): Promise<CompletionRequestOutcome> {
    this.logInfo(input.settings, `sent ${input.details}`)
    const modelStarted = Date.now()
    let response: CompletionModelMessage | undefined
    try {
    ...
    let retryResponse: CompletionModelMessage | undefined
    try {
  ...
  private completionOutcomeFromResponse(input: {
    response: CompletionModelMessage | undefined
    document: vscode.TextDocument
    settings: RemoteSettings
    details: string
    started: number
...
function deterministicCompletionMessage(text: string): CompletionModelMessage {
  return {
    info: {
