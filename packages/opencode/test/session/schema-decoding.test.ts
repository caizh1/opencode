...
describe("Todo.Info", () => {
  ...
  test("round-trip with stable id", () => {
    const input = { id: "todo_1", content: "do a thing", status: "pending", priority: "high" }
    expect(decode(input)).toEqual(input)
  })

  test("Todo.Input accepts missing id", () => {
    const input = { content: "do a thing", status: "pending", priority: "high" }
    expect(decodeUnknown(Todo.Input)(input)).toEqual(input)
  })
})
